...
url = (
f"https://maps.googleapis.com/maps/api/place/nearbysearch/json?" f"location={lat},{lng}&radius={raio}&type=restaurant&key={API_KEY}"
)
...
icon=folium.Icon(color='red', icon='cutlery', prefix='fa')).add_to(mapa) bounds.append([pos['lat'], pos['lng']])
for r in custo_beneficio:
pos = r['geometry']['location']
folium.Marker([pos['lat'], pos['lng']],
tooltip=r['name'],
popup=f"Custo Benefício<br>{r['name']}<br>Nota: {r.get('rating')}", icon=folium.Icon(color='green', icon='cutlery', prefix='fa')).add_to(mapa) bounds.append([pos['lat'], pos['lng']])
...
repo.update_file(existing.path, "Atualização do mapa", content, existing.sha, branch=BRANCH) except:
